...
## D-042: vendor/ lives in a named volume, not the Windows bind mount
Status: Accepted (post-Phase 9)
Decision: `docker-compose.yml` mounts `./backend:/var/www/html` as before, plus a named `vendor` volume over `/var/www/html/vendor`. The dev image installs into it on first start when it is empty, so `docker compose up -d` stays the single start command. Composer must run through `docker compose exec app`.
Why: Local requests took 3–5 s and the test suite ~100 s. Measured inside the container, reading one PHP file through the Windows bind mount cost **63 ms** against **0.009 ms** on the container's own filesystem, and `stat()` 4.9 ms against 0.002 ms. `vendor/` is ~8.3k files and the bulk of what every request loads, so it was the whole cost. After the change: API request ~0.05 s, full suite ~20 s. `docs/TESTING.md` had blamed per-test boot and database work, which the measurement disproved.
Alternatives: moving the repository into the WSL2 filesystem (fastest, but relocates the project and every tool path); a Windows Defender exclusion for the folder (helps, but is a machine setting no repository can carry); enabling OPcache for the CLI (measured again after this change and made no difference, so it stays off).
Consequences: The host `backend/vendor` remains for IDE autocomplete but is no longer what runs, so the two can drift — `composer install` on the host now changes nothing the container sees. Deleting the volume forces a reinstall, which the first start does automatically in about 20 s. Production is unaffected: its image copies the code in and never bind-mounts.
